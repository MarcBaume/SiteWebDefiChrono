...
<div class="popup" >
  <span class="popuptext" id="myPopup">
		  ...
		  <table style="background :Dimgray;">
			<tr>
				<td>
       				 <table  >
						<input type="hidden" name="LoginCompte" id="LoginCompte"   value= '<?php echo $_SESSION['Login'] ?>' />
						<tr style="background :gray;"><td >Nom</td> <td><input type="text" name="nom" id="nomAdd" tabindex="10" style="width:80%;margin:10px"/></td></tr>
						<tr style="background :gray;"><td >Prénom</td> <td> <input  type="text" name="prenom" id="prenomAdd" tabindex="20" style="width:80%;margin:10px"/></td></tr>
						<tr style="background :gray;"><td  >Adresse e-mail</td>  <td> <input type="text" name="email" id="emailAdd" tabindex="40" style="width:80%;margin:10px"/></td></tr>
						<tr style="background :gray;"><td  >Adresse</td> <td>  <input type="text" name="adresse" id="adresseAdd" tabindex="50" style="width:80%;margin:10px"/></td></tr>
						<tr style="padding: 10px; background :gray;"><td >NPA</td>  <td> <input type="text" name="zip" id="zipAdd" tabindex="60" style="width:80%;margin:10px"/></td></tr>
						<tr style="padding: 10px; background :gray;"><td >Localité</td>  <td> <input type="text" name="ville" id="villeAdd"tabindex="70" style="width:80%;margin:10px" /></td></tr>
						<tr style="padding: 10px; background :gray;"><td  >Pays</td>   <td> <input type="text" name="pays" id="paysAdd"tabindex="80" style="width:80%;margin:10px"/></td></tr>	
						<tr style="padding: 10px; background :gray;"><td  >Année de Naissance</td> <td><input type="text" name="dateNaissance" 	id="dateNaissanceAdd" tabindex="90" style="width:80%;margin:10px"  /><td></tr>

						<tr style="padding: 10px; background :gray;"><td  for="club">Club</td><td> <input type="text" name="club" id="clubAdd"tabindex="100" style="width:80%;margin:10px"/></td></tr>
											
						<tr style="padding: 10px; background :gray;"><td >Sexe</td><td><Select name="sexe"   id="sexeAdd" style="width:80%;margin:10px"> 
			...
			<tr>
				<td>
					<button type ="button" style="float:right; margin-right :10px;" onClick="checkForm2(this.form)" title="Validations Informations" data-toggle="tooltip" data-placement="right"><img src="/images/validation.jpg" width="50"></button>
				</td>
			</tr>
		</table>
...
<div id="InformationsCoureurs" style="display:none;">
		...
		<h3>Vérfier les informations du coureur choisi, elles peuvent être modifiées sur <a href="admin/membres.php">votre profil )
...
//******************************************************************************************
	...
	$sql = 'SELECT * FROM Membres  WHERE LoginCompte=\''.$_SESSION['Login'].'\'';//AND Valider =\'1\'';
	//echo $sql;
 	...
 	if ($result && mysqli_num_rows($result) > 0) 
	{
        ...
        if (ArrayCoureurs.length > 1)
        {
            var sel = document.getElementById("Coureur");
            sel.options.add( new Option("Sélectionner un coureur", "", ICounterCoureurs));
            sel.setAttribute("style","background-color:#41e063");
        }

        for(var iCoureur=0; iCoureur<ArrayCoureurs.length; ++iCoureur) 
        {
            var sel = document.getElementById("Coureur");
            sel.options.add( new Option(ArrayCoureurs[iCoureur].Nom + " "+ ArrayCoureurs[iCoureur].Prenom, iCoureur));

        }

        if (ArrayCoureurs.length == 1)
        {
            ChoixCoureurs( document.getElementById("Formulaire"));
        ...
        if ($Aff == 0)
        {
            echo "<p style='font-size:24px;Background-color:#FE7878'>Aucun coureur validé dans ce compte</p>" ;
    ...
    {
        echo "<p style='font-size:24px;Background-color:#FE7878'>Aucun coureur dans ce compte</p>";
    }
...
<img src="images/FilRougeInscription1.png" style="width: 100%" >
	...
	<h3><a href="PasswordForget2023.php"style="font-size:12px;">Mot de passe oublié </a></h3>
  <?php
}
