...
<?php
	...
	{
		header('Location: Resultat2023GeneralJuraDefi.php?NbrEtape='.$Nbr_etape.'&Etape='.$_GET['Etape'].'&DateCourse='.$DateCourse.'&NomCourse='.$NOM_COURSE.''); 
...
<script>
	...
	// Lecture du fichier " info étape "
		...
		Parcours.info =  readJSON(PathFolder +"//Etape"+ PathEtape+ "//Resultats.json");
	...
	<?			}
		...
		function ListCoureurLiveToTable(LivePointPassage, ListCoureurs, LastPointDePassage)
		{
			...
			else
			{
				...
				if (LastPointDePassage)
				{
					
					var TotalCoureur = LivePointPassage.ListCoureursArrivee.length +  LivePointPassage.ListCoureursRestant.length ;
					Title.innerHTML = LivePointPassage.Name + "( Finisher : "+ LivePointPassage.ListCoureursArrivee.length + " / " + TotalCoureur + " )" ;
				}
				else
				{
					Title.innerHTML = "Vu au point de passage : " +LivePointPassage.Name + "( Coureur restant : "+ ListCoureurs.length+ " )" ;
				}
				...
				else
				{
					...
					if (FormTypeClassement.value == "Sexe" )
					{
						LivePointPassage.ListCoureursArrivee.sort((a,b) => a.Coureur.Sexe._Value < b.Coureur.Sexe._Value  ? -1 : 1);

					}
					else 	if (FormTypeClassement.value == "Categorie" )
					{
						console.log(LivePointPassage.ListCoureursArrivee);
						LivePointPassage.ListCoureursArrivee.sort((a,b) => a.Coureur.Categorie.NumCategorie._Value < b.Coureur.Categorie.NumCategorie._Value  ? -1 : 1);
						console.log(LivePointPassage.ListCoureursArrivee);
					}
                                    ...
                                    if ( ListCoureurs[i].Coureur.ListTempsPointPassage.length > 1 )
                                    {
										...
										colonne = document.createElement('td');
												...
												if (FormTypeClassement.value == "Scratch" ||FormTypeClassement.value == "")
												{
													...
													colLoc.style.margin ="1px";
													colLoc.style.fontSize = "14px";
													colLoc.innerText = ListCoureurs[i].Coureur.ListTempsPointPassage[j].PositionCategorie +" ( cat. " +ListCoureurs[i].Coureur.Categorie.NumCategorie._Value +" ) " ;
													RowLoc.appendChild(colLoc);
                                     ...
                                     // Ajout tableau point de passage 
                                        ...
                                        for (let j = 0; j < ListCoureurs[i].Coureur.ListClassementPointPassage.length; j++) 
                                        {
                                            ...
                                            if (FormTypeClassement.value == "Scratch" ||FormTypeClassement.value == "")
                                            {
                                                ...
                                                colLoc.style.margin ="1px";
                                                colLoc.style.fontSize = "14px";
                                                colLoc.innerText = ListCoureurs[i].Coureur.ListClassementPointPassage[j].PositionCategorie +" ( cat. " +ListCoureurs[i].Coureur.Categorie.NumCategorie._Value +" ) " ;
                                                RowLoc.appendChild(colLoc);
