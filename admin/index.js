...
const consoleOutput = (e, response) => {
  ...
  const formattedResponse = (response ? '</div>' +
    ...
    jQuery('.console-content').append('<div class="columns col-oneline"><div class="column col-auto">' + EVENTS[e] + formattedResponse + '</div></div>');
...
jQuery(document).ready(function () {
  ...
  jQuery('#clearButton').click(() => $('.console-content').empty());

  PaymentButton.init(buttonOptions);

  PaymentButton.on('init', function () {
    jQuery('.console-content').append('init event dispatched <br>');
    PaymentButton.create(document.getElementById('paybutton'), payment);
  });
