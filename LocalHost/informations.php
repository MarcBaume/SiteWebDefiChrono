...
<script>
	...
	function funAddSvg(IDSVG, FileName, ColimgEtapePara)
	{
		...
		GraphiqueSVG.name = IDSVG+'image1';
		...
		*																															*
		...
		for (var i = 0; i < ArrayPoint.length; i++)
		{
			...
			if ( LastPoint != null )
			{
				...
				if (i ==1)
				{
					AddPoint ( LastPoint, Point, mymap, IDSVG+'image1', true,false );
				}
				// Point finish
				else if (i == ArrayPoint.length-1 )
				{
					AddPoint ( LastPoint, Point, mymap, IDSVG+'image1', false,true );
				}
				else 
				{
					AddPoint ( LastPoint, Point, mymap, IDSVG+'image1', false,false );
				}
			}
			LastPoint = Point;
		}	
	...
	//UNCTION AJOUT DE POINT SUR LES DESSINS ****/
	function AddPoint(LastPoint, Point ,mymap , GraphiqueSVG, xStart, xFinish)
	{
		...
		// Position de la ligne déniveller 
		var obj1 = new Object()
		obj1.x = TransformDistanceEnPxl(LastPoint.KM) + DecalageStartWidth;
		obj1.y = TransformElevationEnPxl(LastPoint.elevation) + DecalageStartHeight;
		data.push(obj1)
		...
		const svg = d3.select("#"+GraphiqueSVG);
		const margin = 40;
		...
		// 2. Échelles (pour convertir les données en pixels)
		...
		// 3. Générateur de ligne avec LISSAGE (curveMonotoneX)
		const line = d3.line()
			.x(d => xScale(d.x))
			.y(d => yScale(d.y))
			.curve(d3.curveMonotoneX); // <--- L'astuce est ici
		...
		// 5. Optionnel : Ajouter les points réels pour voir la précision
		g.selectAll(".dot")
			.data(data)
			.enter().append("circle")
			.attr("class", "dot")
...
<table style="width:90%; margin-Top:120px">
	...
	<?php $Nbr_etape = intval ($course ["nbr_etape"]);		
	// ******************** POUR COURSE 1 ETAPE **********************************/		
if ($Nbr_etape < 2)
{?>
	...
	<tr  >
		<td   style= "width:50%;background:#BCDDFD; padding: 10px;">
			<table   style= "width:100%;">
				...
				<?php echo  strftime('%A %d %B %Y ',strtotime($course ["Date"]));?>
					</td>
				</tr>
			</table>
		</td>
		...
		<td style= "width:50%;background:#BCDDFD; padding: 10px;">
			<table style= "width:100%;">
				<tr>	
					...
					<td style= "background:#BCDDFD;padding: 10px;">
						<?php echo $course ["Lieu"] ;
						if ( strlen($course ["Emplacement"] ) > 1)     
						{ ?>
							</br></br>
							<?php echo $course ["Emplacement"] ;
    ...
    <td   style= "width:50%;background:#BCDDFD; padding: 10px;" >
		<table  style= "width:100%;">
			<tr>
				...
				<td style= "background:#BCDDFD;padding:10px;">
					<?php
					if(strpos($course ["Site"] , "http") >-1)
					{?>
						<a target="_blank" href=<?php echo $course ["Site"] ?>><?php echo $course ["Site"] ?> </a><?php
					}
					else
					{?>
					<a target="_blank" href=<?php echo "http://www.". $course ["Site"] ?>><?php echo "www.". $course ["Site"] ?> </a><?php
					}?>
				</td>
			</tr>
		</table>
		...
		<table    style= "width:100%;">
			<tr>
				...
				<td style= "background:#BCDDFD;padding:10px;">
					...
					if ($course ["InscriptionExtern"] )
					{
						?> Les inscriptions s'effectue sur le site de l'organisateur
					<?php
					}
					else if ($today >$course ["Date"] )
					{
						echo 'Course Terminé';
					}
					else if ( $today < $course ["DateStartInscription"]  )
					{?>
						les inscriptions pour cette course ne sont pas encore ouverte, elles ouvrent le       <?php echo  strftime('%A %d %B %Y ',strtotime($course ["DateStartInscription"]));

					}
					else if ( $today > $course ["DATE_END_INSCRIPTION"] )
					{
					...
					{
						$Dateend =  date_parse($course ["DATE_END_INSCRIPTION"]);?> 	
						<b>	Inscriptions : </b> </br></br>
						Les inscriptions en ligne sont ouvertes jusqu'au : 
						<?php
							echo strftime('%A %d %B %Y       %H:%M',strtotime($course ["DATE_END_INSCRIPTION"])) ;

						?>

						<?php 

						if (  $course ["NO_INSCRIPTIONS_SUR_PLACE"])
						{
						?>
						...
						if ( strlen(  $course ["FichierInscription"])> 2)
						{
						?><br>
						<?php echo $course ["FichierInscription"]?></br>
						<?php
						}
					}
					if (! strlen($course ["InscriptionExtern"] ))
					{ ?>
		...
		<td style= "width:50%;background:#BCDDFD; padding: 10px;">
			<table  style= "width:100%;">
				<tr >
					...
					<td style= "background:#BCDDFD;padding:10px;">
					<?phpecho '<a href="'.$FileReglement.'"target="_blank">Règlement , Cliquer ici</a>'?>
					</td>
				</tr>
			</table>
		</td>
		...
		<td style= "width:50%;background:#BCDDFD; padding: 10px;">
			<table  style= "width:100%;">
				<tr >
					...
					<td style= "background:#BCDDFD;padding:10px;">
					<?php echo '<a href="'.$FileReglement.'"target="_blank">Programme , Cliquer ici</a>'?>
					</td>
				</tr>
			</table>
		</td>
...
<?php
if ( strlen($course ["Informations"] ) > 1)
{ ?>
  <div class="title"> Informations </Div>
  <Fieldset>

	<?php echo $course ["Informations"] ?> 
  </Fieldset>
	<?php
}
if ( strlen($course ["Video"] ) > 1)
{ 
?>
...
src="<?php echo 'https://www.youtube.com/embed/'.  $course ["Video"]?>">
</iframe>
</Fieldset>
...
<script>
	...
	if (false && document.getElementById("Photos") != null)
	{
		var arFilePhotos = <?php echo json_encode($arfilesPhoto); ?>;
		var index = 0;
		var myVar=setInterval(function(){myTimer()},3000);	
		document.getElementById("Photos").src ="<?php echo $PathFolderPhoto ?>/" +arFilePhotos[index];
	}
function myTimer() {		
