...
//	document.getElementById("TotalReduction").value = TotalReduction;
	...
	string1 = "https://defichrono.payrexx.com/pay?tid="+OrderId+
	"&invoice_number="+OrderId+
	"&invoice_amount="+ Total +"&invoice_currency=1&contact_forename="+
	LoginLight +"&contact_surname="+
	LoginLight2+"&contact_email="+ Login;
...
CalculPrixTotal();
		...
		//	$string1 = "https://defichrono.payrexx.com/pay?tid=".$OrderID."&invoice_number=".$OrderID."&invoice_amount=".  $PrixForm ."&invoice_currency=1&contact_forename=". $LoginLight ."&contact_surname=". $LoginLight2."&contact_email". $_SESSION["Login"];
		 ?>
		<Center>
			<a id="IDValide"  style="background:#00b4ff ;cursor: pointer; Padding:10px ;color:black;  display:none" onclick="ValidInscription()" >
			<u>	Valider mes inscriptions En ligne</u>
			</a>
			 <a id="IDPayresxx" style="background:#00b4ff ; Padding:10px ;color:black; visibility:Visible; display:block" >
				Payer mes inscriptions En ligne
			</a>
		</center>
		...
		<a style="font-size : 14px;">
			* Vos inscriptions sont enregistrées, vous pouvez les payer en ligne, il est toujours possible de payer sur place </br></br>
    ...
    // On affiche chaque entrée une à une
		...
		<p class="title2">Inscriptions effectuées </p>
			...
			{ ?>
			<?
				if ($donnees['Payer'] == "Payer" ||$donnees['Payer'] == "Valider" ||$donnees['Payer'] == "Bon" ||$donnees['Payer'] == "Payé" )
				{
					if ($Background == 1)
					{
						$Background = 0;?>
